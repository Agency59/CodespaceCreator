...
if (envFileExists) {
    ...
    if (currentEnv.GH_CLIENT_ID && currentEnv.GH_CLIENT_SECRET && currentEnv.GH_PRIVATE_KEY && currentEnv.GH_APP_ID && currentEnv.GH_WEBHOOK_SECRET) {
        console.log('Dev app is already set, you can start working on the project.')
        process.exit(0)
    }
}
...
app.get('/redirect', async (req, res, next) => {
    try {
        ...
        const {data: response} = await octokit.request('POST /app-manifests/{code}/conversions', {
            code,
            headers: {
